...
def transform_bls_data(**kwargs):
    ...
    # Perform any additional transformations as needed (if necessary)
    # e.g., Convert 'value' column to numeric type if it's not already
    bls_cleaned['value'] = pd.to_numeric(bls_cleaned['value'], errors='coerce')
    ...
    return bls_file_path  # Return the file path to pass to the next task in the DAG
